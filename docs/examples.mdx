...
<Info>
  Memory is automatically injected into LLM prompts using `user_id` and `unit_id` from `context`.

  Use `app.add_memory` or the memory extraction middleware to populate durable memories.
</Info>
