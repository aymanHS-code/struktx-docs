...
<Info>
  If <code>OPENAI_API_KEY</code> is set and no <code>llm</code> is provided, a sensible default LLM client is selected automatically. Otherwise, a minimal <code>SimpleLLMClient</code> is used.
</Info>
...
app = create(StruktConfig(
  llm=dict(factory="langchain_openai:ChatOpenAI", params={"model": "gpt-4o-mini"}),
  classifier=dict(factory=DefaultLLMClassifier, params={"max_parts": 4, "allowed_types": ["time_service", "general", "memory_extraction"]}),
  memory=MemoryConfig(
...
<Info>
  Memory can be scoped by <code>user_id</code> and <code>unit_id</code>. With <code>augment_llm=True</code>, StruktX automatically injects relevant, scoped memory into prompts.
</Info>
