...
<Info>
  If `OPENAI_API_KEY`is set and no `llm`is provided, a sensible default LLM client is selected automatically. Otherwise, a minimal `SimpleLLMClient`is used.
</Info>
...
<Info>
  Memory can be scoped by `user_id`and `unit_id`. With `augment_llm=True`, StruktX automatically injects relevant, scoped memory into prompts.
</Info>
