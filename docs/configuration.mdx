...
<Info>
  StruktX uses a flexible factory-based configuration system. You can pass factories as callables, classes, instances, or import strings like <code>"module:attr"</code>. Plain dicts for component configs are also accepted and coerced into dataclasses.
</Info>
...
<Note>
  Builder progressively injects <code>llm</code>, <code>memory</code>, and <code>store</code> into factories if their signatures accept them. Additional <code>params</code> are forwarded.
</Note>
...
<Info>
  With <code>augment_llm=True</code>, all LLM calls receive relevant memory snippets automatically. Provide <code>context</code> with <code>user_id</code> and optional <code>unit_id</code> for scoping.
</Info>
...
app.invoke("I currently live in <specify a location>, what is the time there?")
```
